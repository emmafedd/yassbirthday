...
const text = "happy 24 birthday my love ";
let i = 0;
...
// Duck/stop background music while vidio1 is playing
const bgMusic = document.getElementById("music");
const bdayVideo = document.getElementById("birthdayVideo");

if (bdayVideo && bgMusic) {

    let musicWasPlaying = false;

    bdayVideo.addEventListener("play", function () {
        if (!bgMusic.paused) {
            musicWasPlaying = true;
        }
        bgMusic.pause();
        // If you'd rather just lower the volume instead of stopping it,
        // comment the line above and uncomment the one below:
        // bgMusic.volume = 0.1;
    });

    bdayVideo.addEventListener("pause", function () {
        if (musicWasPlaying) {
            bgMusic.play().catch((err) => console.log(err));
        }
    });

    bdayVideo.addEventListener("ended", function () {
        if (musicWasPlaying) {
            bgMusic.play().catch((err) => console.log(err));
        }
    });
